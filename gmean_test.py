...
scores = np.load('results/gmean_results.npy')
...
with open(f"test_results/geometric_mean_results.txt", "w") as f:
    for dataset in range(scores.shape[0]):
        ...
        t_stat_table = tabulate(np.concatenate((names_column, t_stat), axis = 1), headers, floatfmt=".3f")
        p_val_table = tabulate(np.concatenate((names_column, p_val), axis = 1), headers, floatfmt=".3f")
        adv_table = tabulate(np.concatenate((names_column, adv), axis = 1), headers, floatfmt=".3f")
